...
/////////////////////////////////////
// Plugin class
cr.plugins_.ConstructPlayGames = function(runtime)
{
...
{
	/////////////////////////////////////
	var pluginProto = cr.plugins_.ConstructPlayGames.prototype;
	...
	instanceProto.onCreate = function()
	{
		...
		if (this.runtime.isCordova)
		{
			...
			playInterface["setCallback"](function (err, state) {
				if (err)
				{
					//TODO there's no condition for this at the moment
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAccountError, theInst);
				}
				else
				{
					isSignedIn = state;
					if (isSignedIn)
						theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnSignedIn, theInst);
					else
						theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnSignedOut, theInst);
				}
			});
		...
		else
		{
			window["googlePlayLoadCallback"] = function ()
			{
				...
				{
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnLoaded, theInst);
				}
				else
				{
					fireLoadedFirstTick = true;
			...
			window["googlePlaySigninCallback"] = function (auth)
			{
				...
				else if (auth["error"] === "immediate_failed")
				{
					...
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAutoSignInFailed, theInst);
				}
				else
				{
					isSignedIn = false;
					lastError = auth["error"].toString();
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnError, theInst);
				}
			};
	...
	instanceProto.onLayoutChange = function ()
	{
		...
		if (isLoaded)
			this.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnLoaded, this);
		
		if (isSignedIn)
			this.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnSignedIn, this);
	};
		...
		if (this.runtime.isCordova)
		{
			playInterface["getPlayerData"](
				function success (err, playerData) {
					...
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnPlayerDetails, theInst);
				}
			);
		}
		else
	...
	Acts.prototype.SubmitScore = function (leaderboardId, score, tag)
	{
		...
		if (this.runtime.isCordova)
		{
			playInterface["submitScore"](params, function (err)
			{
				...
				{
					lastError = err;
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnScoreSubmitFail, theInst);
					return;
				}
				theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnScoreSubmitSuccess, theInst);
			});
		}
		else
		{
	...
	Acts.prototype.RequestHiScores = function (leaderboardId, collection, timespan, maxresults, type)
	{
		...
		if (this.runtime.isCordova)
		{
			...
			playInterface["getHiScores"](params, function (err, response)
			{
				...
				{
					lastError = getErrorString(err);
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnHiScoreRequestFail, theInst);
					return;
				}
				...
				theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnHiScoreRequestSuccess, theInst);
			});
		}
		else
		{
			gapi["client"]["request"]({
				...
				"callback": function (response)
				{
					...
					// NOTE this value will be 0 in cordova
					hiscores_total = parseInt(response["numScores"], 10) || 0;
	...
	Acts.prototype.RequestAchievements = function (which)
	{
		...
		if (this.runtime.isCordova)
		{
			playInterface["getAchievements"](params, function (err, response)
			{
				...
				{
					lastError = getErrorString(err);
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAchievementsRequestFail, theInst);
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAchievementsMetadataFail, theInst);
					return;
				}
				...
				theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAchievementsMetadataSuccess, theInst);
				theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAchievementsRequestSuccess, theInst);
			});
		}
		else
		{
	...
	Acts.prototype.RevealAchievement = function (id)
	{
		...
		if (this.runtime.isCordova)
		{
			...
			playInterface["revealAchievement"](params, function (err)
			{
				...
				theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAchievementRevealed, theInst);
			});
		}
		else
		{
	...
	Acts.prototype.UnlockAchievement = function (id)
	{
		...
		if (this.runtime.isCordova)
		{
			...
			playInterface["unlockAchievement"](params, function (err)
			{
				...
				{
					achievement_trigger_id = id;
					// NOTE we don't know if this has actually happened, or if
					//  the server has updated so change the value and hope it has
					a["achievementState"] = "unlocked";
					theInst.runtime.trigger(cr.plugins_.ConstructPlayGames.prototype.cnds.OnAchievementUnlocked, theInst);
				}
